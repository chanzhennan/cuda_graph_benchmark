...
#include "cuda_graph/cuda_graph.cuh"
...
template <typename T>
class CudaGraph : public benchmark::Fixture {
 public:
  void callKernel(benchmark::State &state) {
    // call kernel
    testAdd3(d_array, d_result, dataSize, stream);
...
#define BENCHMARK_GRAPH3_OP(name, dType)                               \
  BENCHMARK_TEMPLATE_DEFINE_F(CudaGraph, name, dType)                  \
  (benchmark::State & st) {                                            \
    for (auto _ : st) {                                                \
      callKernel(st);                                                  \
    }                                                                  \
  ...
  BENCHMARK_REGISTER_F(CudaGraph, name)                                \
      ->Unit(benchmark::kMillisecond)                                  \
      ->RangeMultiplier(2)                                             \
      ->Range(512, 1024);

#define BENCHMARK_GRAPH3_OP_TYPE(dType) \
  BENCHMARK_GRAPH3_OP(Graph_##dType, dType)

BENCHMARK_GRAPH3_OP_TYPE(float)
